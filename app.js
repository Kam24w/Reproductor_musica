...
const modalCancelBtn2 = document.getElementById("modal-cancel-2");
const newPlaylistInput = document.getElementById("new-playlist-input");
const newPlaylistBtn = document.getElementById("new-playlist-btn");
const itunesSearchLinkEl = document.getElementById("itunes-search-link");
...
async function importItunesPlaylistFromQuery(query) {
...
function normalizeItunesArtwork(artworkUrl) {
    if (!artworkUrl)
        return "";
...
function inferRecommendationSeedFromPlaylist(pl) {
    ...
    if (name.includes("pop"))
        addSeed(UI_TEXT.common.byGenre, "pop hits");
    else if (name.includes("rock"))
        addSeed(UI_TEXT.common.byGenre, "rock classics");
    else if (name.includes("latin") || name.includes("latino"))
        addSeed(UI_TEXT.common.byGenre, "latin hits");
    else if (name.includes("chill") || name.includes("relax"))
        addSeed(UI_TEXT.common.byGenre, "chill vibes");
    else if (name.includes("workout") || name.includes("gym") || name.includes("fitness"))
        addSeed(UI_TEXT.common.byGenre, "workout mix");
    else if (name.includes("indie"))
        addSeed(UI_TEXT.common.byGenre, "indie pop");
    else if (name.includes("dance") || name.includes("party"))
        addSeed(UI_TEXT.common.byGenre, "dance hits");
...
function isAudioPreviewTrack(track) {
...
function formatTrackDuration(track) {
    ...
    if (currentTrackDuration > 0)
        return formatTime(currentTrackDuration);
...
function syncFooterProgressUI() {
    const fill = document.querySelector(".footer-bar-fill");
...
function ensureAccountMenuEl() {
  ...
  `;
    ...
    saveBtn.addEventListener("click", () => {
        ...
        if (!name) {
            if (error)
                error.textContent = UI_TEXT.account.saveNameRequired;
            nameInput.focus();
            return;
        }
...
function ensureSearchModalEl() {
  ...
  `;
    ...
    const runSearch = async () => {
        ...
        if (!query) {
            if (error)
                error.textContent = UI_TEXT.search.emptyQuery;
            input.focus();
            return;
        }
        if (error)
...
function openAccountMenu() {
    ...
    if (!accountMenuOutsideHandler) {
        accountMenuOutsideHandler = (event) => {
            ...
            if (!target || !chipEl || !menuEl)
                return;
            if (chipEl.contains(target) || menuEl.contains(target))
                return;
            closeAccountMenu();
        };
    }
    if (!accountMenuKeyHandler) {
        accountMenuKeyHandler = (event) => {
            if (event.key === "Escape") {
                closeAccountMenu();
            }
        };
...
function ensureAccountSetupModal() {
...
async function promptAccountSetup() {
    const overlay = ensureAccountSetupModal();
    const nameInput = document.getElementById("account-name-input");
    const themePicker = document.getElementById("account-theme-picker");
    ...
    await new Promise(resolve => {
        const getSelectedTheme = () => {
            const active = themePicker.querySelector(".account-theme-option.active");
...
function renderSidebar() {
    ...
    `;
        li.addEventListener("click", e => {
            if (e.target.closest(".sidebar-pl-delete"))
                return;
  ...
  `;
    ...
    document.querySelectorAll(".track-row").forEach(row => {
        row.addEventListener("click", e => {
            if (e.target.closest(".track-delete-btn") || e.target.closest(".track-favorite-btn"))
                return;
...
function focusInlineHomeSearch(selectText = false) {
...
function renderHomeContent() {
  ...
  `;
    ...
    document.querySelectorAll("[data-favorite-track]").forEach(btn => {
        btn.addEventListener("click", e => {
            e.stopPropagation();
            const playlistName = btn.closest("[data-playlist-name]")?.dataset.playlistName;
    ...
    document.querySelectorAll(".home-live-row .home-result-play").forEach(btn => {
...
function renderHomePlaylistCard(pl) {
    return `
...
function renderHomeTrackCard(result) {
    return `
...
function renderHomeSearchResult(result) {
...
function renderLiveItunesResult(track, cover) {
    return `
...
function renderHomeRecentCard(pl) {
...
function matchesHomeSearch(name, tracks, query) {
...
function renderTrackRow(track, index, pl) {
...
function renderQueueUpcomingNodes(pl) {
    if (!pl.head) {
...
function renderFooter() {
  ...
  `;
    ...
    document.getElementById("fc-prev")?.addEventListener("click", () => {
        ...
        if (t) {
            currentProgress = 0;
            currentTrackDuration = t.duration;
            if (player.isPlaying)
                startProgress();
        }
        renderAll();
...
function setupScrollFallbackHandlers() {
...
newPlaylistBtn.addEventListener("click", () => {
    ...
    if (!name) {
        showToast(UI_TEXT.playlist.writeName, "error");
        return;
    }
    try {
...
newPlaylistInput.addEventListener("keydown", e => { if (e.key === "Enter")
...
function handleDeletePlaylist(name) {
    ...
    if (player.playlists.length <= 1) {
        showToast(UI_TEXT.playlist.noDeleteOnlyOne, "error");
        return;
    }
...
function applyStaticPageTexts() {
    ...
    if (homeNavLabel)
        homeNavLabel.textContent = UI_TEXT.staticPage.navHome;
    const searchNavLabel = itunesSearchLinkEl?.querySelector("span");
    if (searchNavLabel)
        searchNavLabel.textContent = UI_TEXT.staticPage.navSearch;
    const libraryTitle = document.querySelector(".sidebar-library-header span");
    if (libraryTitle)
        libraryTitle.textContent = UI_TEXT.staticPage.libraryTitle;
    if (newPlaylistInput)
        newPlaylistInput.placeholder = UI_TEXT.staticPage.newPlaylistPlaceholder;
    if (newPlaylistBtn)
        newPlaylistBtn.setAttribute("title", UI_TEXT.staticPage.newPlaylistTitle);
    const modalTitle = document.querySelector("#modal .modal-header h2");
    if (modalTitle)
        modalTitle.textContent = UI_TEXT.staticPage.modalTitle;
    const modalLabels = document.querySelectorAll("#modal .modal-field label");
    if (modalLabels[0])
        modalLabels[0].innerHTML = `${UI_TEXT.staticPage.modalSongLabel} <span class="req">*</span>`;
    if (modalLabels[1])
        modalLabels[1].innerHTML = `${UI_TEXT.staticPage.modalArtistLabel} <span class="req">*</span>`;
    if (modalLabels[2])
        modalLabels[2].innerHTML = `${UI_TEXT.staticPage.modalDurationLabel} <small>mm:ss</small>`;
    if (modalLabels[3])
        modalLabels[3].innerHTML = `${UI_TEXT.staticPage.modalPositionLabel} <small>${UI_TEXT.staticPage.modalOptional}</small>`;
    ...
    if (modalCancelBtn2)
        modalCancelBtn2.textContent = UI_TEXT.staticPage.modalCancel;
    if (modalSubmitBtn)
        modalSubmitBtn.textContent = UI_TEXT.staticPage.modalAdd;
}
function mapDomainErrorToMessage(error) {
    const code = error instanceof Error ? error.message : "";
    if (code === "playlist_name_required")
        return UI_TEXT.playlist.writeName;
    if (code === "playlist_name_exists")
        return UI_TEXT.playlist.nameExists;
...
// ── SVG Icons ─────────────────────────────────────────────
const musicNoteIcon = (s) => `<svg width="${s}" height="${s}" viewBox="0 0 24 24" fill="currentColor"><path d="M12 3v9.28A4.98 4.98 0 0010 12c-2.76 0-5 2.24-5 5s2.24 5 5 5 5-2.24 5-5V7h4V3h-7z"/></svg>`;
const playIcon = (s) => `<svg width="${s}" height="${s}" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>`;
const pauseIcon = (s) => `<svg width="${s}" height="${s}" viewBox="0 0 24 24" fill="currentColor"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg>`;
const plusIcon = (s) => `<svg width="${s}" height="${s}" viewBox="0 0 24 24" fill="currentColor"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>`;
const listIcon = (s) => `<svg width="${s}" height="${s}" viewBox="0 0 24 24" fill="currentColor"><path d="M4 10.5h2v-2H4v2zm0 5h2v-2H4v2zM4 5.5h2v-2H4v2zM8 10.5h12v-2H8v2zm0 5h12v-2H8v2zM8 3.5v2h12v-2H8z"/></svg>`;
const closeIcon = (s) => `<svg width="${s}" height="${s}" viewBox="0 0 24 24" fill="currentColor"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg>`;
const trashIcon = (s) => `<svg width="${s}" height="${s}" viewBox="0 0 24 24" fill="currentColor"><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/></svg>`;
const clockIcon = (s) => `<svg width="${s}" height="${s}" viewBox="0 0 24 24" fill="currentColor"><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67V7z"/></svg>`;
